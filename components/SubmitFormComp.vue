<template>
  <div>
    ...
    <form v-else class="max-w-md mx-auto border p-3" @submit.prevent="submit()">
      <label class="font-bold text-2xl">Add Employee</label>
      ...
      <div class="flex justify-between">
        <button type="submit" class="bg-blue-800 text-white p-2 m-2 rounded">submit</button>
        <button @click="cancle()" class="bg-red-800 text-white p-2 m-2 rounded">cancle</button>
      </div>
    </form>
  </div>
</template>
...
const employee: Ref<Employee> = ref({id:"",firstName:"",lastName:"",email:"",dateOfBirth:"",family:[{name:"",dateOfBirth:"",relation:"son"}]});
const loading=ref(false);
async function submit(){
  loading.value=true;
  const Employees=useEmployees()
  try{
    await Employees.createEmployee(employee.value) 
  }catch(e){
    console.log(e)
  }
  loading.value=false;
...
function addFamilly() {
  employee.value.family.push({ name: "", dateOfBirth: "", relation: "son" });
}
